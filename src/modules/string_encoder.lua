...
local function caesarCipher(data, offset)
    ...
    while i <= #data do
        ...
        -- preserve \27 escape sequences intact
...
function StringEncoder.process(code)
    ...
    local random_isvalid_name   = generateRandomName()
    ...
    local random_newbyte_name   = generateRandomName()
    local random_i_name         = generateRandomName()

    -- Single definition of isValidChar, then decrypt function
    local decode_function = [[
local function ]] .. random_isvalid_name .. [[(]] .. random_byte_name .. [[)
    return (]] .. random_byte_name .. [[ >= 48 and ]] .. random_byte_name .. [[ <= 57)
        or (]] .. random_byte_name .. [[ >= 65 and ]] .. random_byte_name .. [[ <= 90)
        or (]] .. random_byte_name .. [[ >= 97 and ]] .. random_byte_name .. [[ <= 122)
end

local function ]] .. random_decrypt_name .. [[(]] .. random_code_name .. [[, ]] .. random_offset_name .. [[)
    local ]] .. random_result_name .. [[ = {}
    for ]] .. random_i_name .. [[ = 1, #]] .. random_code_name .. [[ do
        local ]] .. random_byte_name .. [[ = ]] .. random_code_name .. [[:byte(]] .. random_i_name .. [[)
        if ]] .. random_isvalid_name .. [[(]] .. random_byte_name .. [[) then
            local ]] .. random_newbyte_name .. [[
            if ]] .. random_byte_name .. [[ >= 48 and ]] .. random_byte_name .. [[ <= 57 then
                ]] .. random_newbyte_name .. [[ = ((]] .. random_byte_name .. [[ - 48 - ]] .. random_offset_name .. [[ + 10) % 10) + 48
            elseif ]] .. random_byte_name .. [[ >= 65 and ]] .. random_byte_name .. [[ <= 90 then
                ]] .. random_newbyte_name .. [[ = ((]] .. random_byte_name .. [[ - 65 - ]] .. random_offset_name .. [[ + 26) % 26) + 65
            elseif ]] .. random_byte_name .. [[ >= 97 and ]] .. random_byte_name .. [[ <= 122 then
                ]] .. random_newbyte_name .. [[ = ((]] .. random_byte_name .. [[ - 97 - ]] .. random_offset_name .. [[ + 26) % 26) + 97
            end
            table.insert(]] .. random_result_name .. [[, string.char(]] .. random_newbyte_name .. [[))
        else
    ...
    -- protect escaped quotes before processing
    code = code:gsub('\\"', '\1\2'):gsub("\\'", '\2\1')

    code = code:gsub("(['\"])(.-)%1", function(quote, str)
        if type(str) == "string" then
            str = str:gsub('\1\2', '\\"'):gsub('\2\1', "\\'")
            local offset = math.random(1, 9)
            if str:match("%a") then
                offset = math.random(1, 25)
            end
            local encoded_str = caesarCipher(str, offset)
            return string.format("%s(%s%s%s, %d)", random_decrypt_name, quote, encoded_str, quote, offset)
        else
            return str
        end
    end)

    -- restore any missed escapes
    code = code:gsub('\1\2', '\\"'):gsub('\2\1', "\\'")
